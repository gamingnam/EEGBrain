...
class Graph:
    def __init__(self, board_shim):
        ...
        self.win = pg.GraphicsLayoutWidget(title='BrainFlow Plot', size=(1200, 800), show=True)
    ...
    def update(self):
        ...
        for count, channel in enumerate(self.exg_channels):
            ...
            if data.shape[1] > self.psd_size:
                ...
                # plot bands
                avg_bands[0] = avg_bands[0] + DataFilter.get_band_power(psd_data, 0.5, 4.0)
                avg_bands[1] = avg_bands[1] + DataFilter.get_band_power(psd_data, 4.0, 8.0)
                avg_bands[2] = avg_bands[2] + DataFilter.get_band_power(psd_data, 8.0, 12.0)
                avg_bands[3] = avg_bands[3] + DataFilter.get_band_power(psd_data, 12.0, 30.0)
                avg_bands[4] = avg_bands[4] + DataFilter.get_band_power(psd_data, 30.0, 80.0)
